    ...
    public function postUpdateSlide(Request $req, $id){
        if (Auth::check() && Auth::user()->level == 1) {
            ...
            if ($req->hasFile('image')) {
                ...
                $image_path = "source/image/slide/" . $getHA->image;
                if (File::exists($image_path)) {
                    File::delete($image_path);
                }
                if ($req->hasFile('image')) {
...
/*-----------------------------------------------NSX--------------------------------------------------------------------*/
    ...
    public function DelAdmin_NSX($id, Request $req)
    {
        ...
        if(empty($sp1[0])){
            ...
            $image_path = "source/image/type_product/" . $image->image;
            if (File::exists($image_path)) {
                File::delete($image_path);
            }
    ...
    public function AddAdmin_NSX(Request $req){
        ...
        if ($req->hasFile('image_file')) {
            ...
            $new_img = $name_img.rand(0,99).'.'.$file->getClientOriginalExtension();
            $filename =time() .'.'. $new_img;
            $file->move('source/image/type_product/', $filename);
            $nsx->image = $filename;
        }else{
            return $req;
            $nsx->image = '';
    ...
    public function postUpdateSp(Request $req, $id){
        if (Auth::check()) {
            ...
            if ($req->hasFile('image')) {
                ...
                $image_path = "source/image/product/" . $getHT->image;
                if (File::exists($image_path)) {
                    File::delete($image_path);
                }
                 if ($req->hasFile('image')) {
...
/*-----------------------------------------------Lang---------------------------------------------------------------*/
    ...
    public function getCoupon(Request $req){
        ...
        if (Auth::check()) {
            ...
            return view('admin.QL_coupon', compact('coupons', 'today', 'coupon_send_new','url_canonical','month_now','day_now','year_now','sendcou'));
